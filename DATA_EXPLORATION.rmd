...
```{r}
# How many samples are teken from each patient
combined_dataset %>%
...
```
INTERPRETATION:
The histogram and box plot of all protein abundances clearly show a large number of outliers, with a long tail to the right. To make this data less skewed,we will perform a log transformation
```{r}
...
GOAL: remove proteins that almost never differ between individuals/conditions.
HOW?
...
```
INTERPRETATION:
The data is analyzed for cases and controls. It is important to note that a person in WAVE 1 could have been considered a control, but later developed COVID-19 and is therefore included as a case in WAVE 2. The filtering was performed per measurement, not per person. We know that this method fails to capture biological differences and differences between cases and controls, and therefore the test is not fully accurate. We choose to work independently of patients, but we recognize that this must be taken into account.

#8 Normalisation; log transformation
...
#11 DEA
Find the protein that is most differentially expressed in the COVID-cases, in comparison with the negative controles. We will visualise this with a vulcano plot
