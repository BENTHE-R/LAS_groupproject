...
```{r}
outlier_list <- sapply(combined_dataset_protein[,2:7241], function(x) {
...
```
INTERPRETATION:
The histogram and box plot of all protein abundances clearly show a large number of outliers, with a long tail to the right. To make this data less squid,we will perform a log transformation
```{r}
