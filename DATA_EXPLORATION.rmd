...
```
INTERPRETATION:
The histogram and box plot of all protein abundances clearly show a large number of outliers, with a long tail to the right. To make this data less skewed,we will add a cut-off value
...
```
INTERPRETATION: The histogram and boxplot indicate that there is still a large number of outliers present. Although applying a cut-off value reduces the numbers of outliers, we will proceed without using the cut-off to avoid introducing bias into the analysis.
...
low_variance <- combined_dataset_protein %>% 
  ...
  filter(coefficient_of_variance < 10) %>%
  arrange(coefficient_of_variance)
low_variance
```
INTERPRETATION:
