...
library(skimr)
skim(features_somameres)
skim
```

str() is used to display the data types. 
```{r}
str(features_somameres)
...
str(combined_dataset_features)
levels(combined_dataset_features$WHO_temp_severity)
```
...
duplicated_rownames <- duplicated(features_somameres)
...
```{r}
unique(features_somameres$col_check)
```
...
```{r}
feature_somameres <- features_somameres %>% select(-c(col_check))
...
```{r}
# How many samples are taken from each patient
combined_dataset %>%
...
```{r}
combined_dataset[combined_dataset == ""] <- NA
...
transposed_df <- t(combined_dataset_features)
transposed_df <- as.data.frame(t(combined_dataset_features))
...
```
INTERPRETATION:
After analysing the data, it appears to be a bias. Whenever there is an NA in one of the columns, the other columns also have an NA. This results in the same number of missing values for WHO_severity, fatal_disease, radiology_evidence_covid, and time_from_first_positive_swab.
...
```
INTERPRETATION:
The histogram and box plot of all protein abundances clearly show a large number of outliers, with a long tail to the right. To make this data less skewed, we will try implement a cut-off value
...
```
INTERPRETATION: The histogram and boxplot indicate that there is still a large number of outliers present. Although applying a cut-off value reduces the number of outliers, we will proceed without using the cut-off to avoid introducing bias into the analysis.

#7 Filtering

The goal of filtering is to investigate if there are proteins or patient-meta data that has almost no variation.

#7.1 Filtering patient-meta data
```{r}
# We first count how often each category occurs. Then, we take the most common category and divides it by the total number of people. This tells you what percentage of the dataset consists of the largest category. If the dominance is higher than 80%, you know that one category strongly dominates, and therefore there is little variation.
...
sapply(combined_dataset_features[2:17], dominantie)
...
```{r}
# proteins with a coefficient_of_variance lower than 10%
low_variance <- combined_dataset_protein %>% 
  pivot_longer(!X, names_to = "prots", values_to = "abundances") %>%
  group_by(prots) %>%
  summarise(coefficient_of_variance = sd(abundances, na.rm = TRUE) / mean(abundances, na.rm = TRUE) * 100) %>%
  arrange(desc(coefficient_of_variance))
low_variance


```
INTERPRETATION:
The data is analyzed for cases and controls. It is important to note that a person in WAVE 1 could have been considered a control, but later developed COVID-19 and is therefore included as a case in WAVE 2. We opted to analyse all samples independently and treat each sample as a separate observation. As a result, biological variation within individuals is not accounted for. We chose to proceed with patient-independent analyses for consistency with the dataset structure, but we acknowledge this limitation and consider it when interpreting the results.
...
sample_totals <- rowSums(combined_dataset_protein [2:7241], na.rm=TRUE)
...
```
INTERPRETATION:
In this figure, we compared the total intensity per sample. The median is the average value of all intensities. We would therefore expect all values to roughly converge around this dotted line, but that is not the case. Some samples are much higher or lower than others, meaning normalisation is necessary.

We selected a random protein, such as LRRC15, and noticed that its abundance is not normally distributed, which necessitates the use of a log transformation. To evaluate the effect of this transformation, we generated histograms for a single protein before and after log transformation. This comparison illustrates the change in distribution. Following this assessment, the log transformation was applied to all protein measurements.

histogram BEFORE transformation
```{r}

ggplot(combined_dataset_protein, aes(x = combined_dataset_protein$LRRC15)) +
  geom_histogram(bins = 100, fill = "steelblue", color = "black", aplha = 0.7) +
  labs(title = "Histogram of LRRC15 before log-transformation",
       x = "LRRC15 protein abundance", y = "Frequency") +
  theme_minimal()
...
```{r}
ggplot(combined_dataset_protein, aes(x = log(combined_dataset_protein$LRRC15))) +
  geom_histogram(bins = 100, fill = "steelblue", color = "black", aplha = 0.7) +
  labs(title = "Histogram of LRRC15 after log-transformation",
...
# heatmap standarsised protein abundances + row annotations on WHO_temp_severity
annotation_WHO_temp_severity = rowAnnotation(COVID_severity = combined_dataset_features$WHO_temp_severity,
                                             col = list(COVID_severity = c("NEGATIVE"="#ef476f", "mild"="#ffd166", "moderate"="#06d6a0", "severe"="#118ab2", "critical"="#073b4c" )))
Heatmap(protein_scaled, name="Scaled protein abundance", use_raster = FALSE, show_row_names = FALSE, show_column_names = FALSE, right_annotation = annotation_WHO_temp_severity,column_title = "log-transformed scaled protein abundance", row_title = "patient samples")
```
INTERPRETATION:
The heatmap reveals extensive heterogeneity in plasma protein expression across samples, with clear clusters of proteins showing coordinated up- or down-regulation. Notably, the NEGATIVE severity group occupies a distinct region of the expression space, characterized predominantly by lower scaled protein abundance compared to COVID-19–positive samples. In contrast, mild to critical cases display progressively stronger deviations from this baseline, with several protein modules exhibiting severity-related gradients. These patterns indicate that COVID-19 infection, particularly increasing clinical severity, is associated with broad, systematic alterations in the plasma proteome.

#10 DEA
Find the protein that is most differentially expressed in the COVID-cases, in comparison with the negative controls. We will visualise this with a vulcano plot
