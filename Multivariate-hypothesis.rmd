...
We aim to investigate whether COVID-19 severity influences COL11A2 expression, whether patient sex affects COL11A2 expression and whether there is an interaction between sex and disease severity in determining COL11A2 levels. The hypotheses are formulated as follows:
-	Null hypothesis (H0): The expression of COL11A2 will not be influenced by the sex of the patient and the Covid-19 severity
-	Alternative hypothesis (H1): The expression of COL11A2 will be influenced by the sex of the patient and the Covid-19 severity
...
```
We also looked at the distribution of COL11A2 in male and female patients. We noticed that there was not big difference between the two groups.
...
data_multivariate %>%
  ...
  geom_qq_line() +
  facet_wrap(~severity) +
  labs( title= "Normalization of severity" )
```
...
data_multivariate %>%
  ...
  geom_qq_line() +
  facet_wrap(~sex)+
  labs( title= "Normalization of sex" )
```
...
- Outliers: There are a few present
...
```
INTERPRETATION:
Tukey post-hoc comparisons showed that all severity groups (mild, moderate, severe and critical) exhibited significantly lower COL11A2 expression compared with the negative group (all p < 0 .001). Furthermore, a clear decrease in expression was observed from mild to moderate, severe and critical disease (all p < 0 .05). The difference between moderate and severe as well as the difference between moderate and critical is also significant. This can however be due to a limited sample size of the severe and critical group.
